...
class Vec3:
    ...
    def dot(self, other: 'Vec3') -> float: return np.dot(self.v, other.v).item()
    ...
    @classmethod
    def zero(cls): return cls(np.array([0, 0, 0], dtype=np.float32))
    @classmethod
    def one(cls): return cls(np.array([1, 1, 1], dtype=np.float32))
