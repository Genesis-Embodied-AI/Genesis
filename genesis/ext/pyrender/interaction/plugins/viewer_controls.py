...
class ViewerDefaultControls(BaseViewerInteraction):
    ...
    def __init__(
        ...
        self.keybindings: Keybindings = Keybindings(
            toggle_keyboard_instructions=pyglet.window.key.I,
            record_video=pyglet.window.key.R,
            save_image=pyglet.window.key.S,
            reset_camera=pyglet.window.key.Z,
            camera_rotation=pyglet.window.key.A,
            shadow=pyglet.window.key.H,
            face_normals=pyglet.window.key.F,
            vertex_normals=pyglet.window.key.V,
            world_frame=pyglet.window.key.W,
            link_frame=pyglet.window.key.L,
            wireframe=pyglet.window.key.D,
            camera_frustum=pyglet.window.key.C,
            fullscreen_mode=pyglet.window.key.F11,
        )
        if options and options.keybindings:
            self.keybindings.apply_override_mapping(options.keybindings)

        self._display_instr = False
        self._instr_texts = (
            ["> [i]: show keyboard instructions"],
            ["< [i]: hide keyboard instructions"] + self.keybindings.as_instruction_texts(
                padding=3, exclude=("toggle_keyboard_instructions")),
        )
    ...
    def on_key_press(self, symbol: int, modifiers: int) -> EVENT_HANDLE_STATE:
        ...
        if symbol == self.keybindings.camera_rotation:
        ...
        elif symbol == self.keybindings.fullscreen_mode:
        ...
        # H toggles shadows
        elif symbol == self.keybindings.shadow:
        ...
        elif symbol == self.keybindings.world_frame:
        ...
        elif symbol == self.keybindings.link_frame:
        ...
        elif symbol == self.keybindings.camera_frustum:
        ...
        elif symbol == self.keybindings.face_normals:
        ...
        elif symbol == self.keybindings.vertex_normals:
        ...
        elif symbol == self.keybindings.record_video:
        ...
        elif symbol == self.keybindings.save_image:
            self.viewer._save_image()

        # D toggles through wireframe modes
        elif symbol == self.keybindings.wireframe:
        ...
        elif symbol == self.keybindings.reset_camera:
            self.viewer._reset_view()

        # I toggles instruction display
        elif symbol == self.keybindings.toggle_keyboard_instructions:
            self._display_instr = not self._display_instr

        # P reloads shader program
        elif symbol == self.keybindings.reload_shader:
