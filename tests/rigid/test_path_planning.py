...
def _max_true_penetration(scene, entity, qpos_waypoints, obstacles):
    # Ground truth through the real collider, independent of the planner's sphere proxy. Mutates the scene, so
    # callers restore the configuration afterward.
...
def test_plan_to_qpos_goal_avoids_obstacles(n_envs, show_viewer, tol):
    ...
    assert bool((path.dofs_vel.abs() <= 2.0 + 1e-5).all())
    ...
    assert bool((path_fixed.dt > 0).all())
    ...
    assert bool(path_line.is_valid.all())
    ...
    assert not bool(path_bad.is_valid.any())
    assert_equal(franka.get_qpos(), qpos_before)
    ...
    if n_envs > 0:
        ...
        assert bool(path_multi.is_valid.all())
        ...
        assert path_sub.qpos.shape[1] == 1 and bool(path_sub.is_valid.all())

    # Ground truth: sweeping the returned waypoints through the real collider never penetrates the obstacles.
    max_penetration = _max_true_penetration(scene, franka, path.qpos, (pillar,))
    assert max_penetration < 5e-3


@pytest.mark.required
@pytest.mark.parametrize("n_envs", [0, 2])
def test_plan_path_with_attached_entity(n_envs, show_viewer, tol):
    scene = gs.Scene(
        viewer_options=gs.options.ViewerOptions(
            camera_pos=(2.5, 1.5, 1.5),
...
@pytest.mark.required
@pytest.mark.parametrize("n_envs", [0, 2])
def test_plan_path_clearance_and_narrow_passage(n_envs, show_viewer, tol):
    scene = gs.Scene(
        viewer_options=gs.options.ViewerOptions(
            camera_pos=(2.5, 1.5, 1.5),
    ...
    assert bool(path.is_valid.all())
    max_penetration = _max_true_penetration(scene, franka, path.qpos, (wall,))
    assert max_penetration < 5e-3
    ...
    if bool(path_margin.is_valid.all()):
        collider_state = scene.rigid_solver.collider._collider_state
    ...
    assert not bool(path_impossible.is_valid.any())
