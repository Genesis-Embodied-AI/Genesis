...
class SkeletonHumanoidEnv(GenesisLocoBaseEnv):
    ...
    def _setup_action_spec(self):
        """Setup action specification based on available Genesis DOFs"""
        print(f"Available DOF names: {self.dof_names}")
        
        # Create action spec based on actually available joints (excluding root freejoint and intermediate body joints)
        intermediate_joints = ['femur_r_joint', 'femur_l_joint', 'torso_joint', 'humerus_r_joint', 'humerus_l_joint', 'hand_r_joint', 'hand_l_joint']
        available_joints = [name for name in self.dof_names if name != 'root' and name not in intermediate_joints]
        
        # Map Genesis joint names back to LocoMujoco action names
        joint_to_action_map = {
            # Lumbar spine
            "lumbar_extension": "mot_lumbar_ext",
            "lumbar_bending": "mot_lumbar_bend", 
            "lumbar_rotation": "mot_lumbar_rot",
            ...
            # Right arm
            "arm_flex_r": "mot_shoulder_flex_r",
            "arm_add_r": "mot_shoulder_add_r",
            "arm_rot_r": "mot_shoulder_rot_r",
            "elbow_flex_r": "mot_elbow_flex_r",
            "pro_sup_r": "mot_pro_sup_r",
            "wrist_flex_r": "mot_wrist_flex_r",
            "wrist_dev_r": "mot_wrist_dev_r",
            
            # Left arm
            "arm_flex_l": "mot_shoulder_flex_l",
            "arm_add_l": "mot_shoulder_add_l", 
            "arm_rot_l": "mot_shoulder_rot_l",
            "elbow_flex_l": "mot_elbow_flex_l",
            "pro_sup_l": "mot_pro_sup_l",
            "wrist_flex_l": "mot_wrist_flex_l",
            "wrist_dev_l": "mot_wrist_dev_l",
        }
        
        # Build action spec from available joints
        self.action_spec = []
        for joint_name in available_joints:
            if joint_name in joint_to_action_map:
                action_name = joint_to_action_map[joint_name]
        ...
        print(f"Available joints: {len(available_joints)}, Mapped actions: {self.num_skeleton_actions}")
    
    def _setup_torque_control(self):
        """Setup pure torque control (zero PD gains)"""
        # Set zero gains for pure torque control
        ...
        # Set torque limits (LocoMujoco uses ±1.0 scaled by gear ratios)
        torque_limit = 87.0  # Standard LocoMujoco torque limit
    ...
    def _apply_actions(self, actions: torch.Tensor):
        """Apply torque actions directly to joints"""
        # Clamp actions to [-1, 1] range (LocoMujoco standard)
        # actions = torch.clamp(actions, -1.0, 1.0)
        
        # Create proper action-to-joint mapping
        torques = torch.zeros((self.num_envs, self.num_dofs), device=self.device)
        
        # Map each action to its corresponding joint DOF
        for i, action_name in enumerate(self.action_spec):
            if i < actions.shape[-1]:  # Ensure we don't exceed action dimensions
                joint_name = self._get_joint_name_from_action(action_name)
    ...
    def _get_joint_name_from_action(self, action_name: str) -> str:
        """Map action name to corresponding joint name in XML"""
        # Mapping from LocoMujoco action names to Genesis joint names
        action_to_joint_map = {
            # Lumbar spine
        ...
        return action_to_joint_map.get(action_name, action_name)
    ...
    def _get_joint_index(self, joint_name: str) -> Optional[int]:
        ...
        try:
            ...
            print(f"Warning: Joint '{joint_name}' not found in DOF names: {self.dof_names}")
            return None
