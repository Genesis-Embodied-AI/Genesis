...
- **Never let a kernel body be duplicated - compile time is ~quadratic in a kernel's inlined size.** Duplication comes as a second `qd.func` call site, a `for i in qd.static(range(N))` (N copies of the body: fine for a few vector components, never for an expensive body nor to lift a loop out of a thread - use `qd.graph_do_while`), or a `qd.template()` used as a loop bound / phase index; a template merely gating two arms is free. Neither a shared wrapper nor `qd.func(is_real_function=True)` escapes it. To collapse, in order: extend an existing loop's domain; make the difference data (indices/bounds/scalars, one body); a request slot issuing one work item per pass; merge the owning phases with the phase as a RUNTIME argument. Splitting a phase into its own kernel is sub-additive, so prefer a split over tangled control flow. Verify by bit-identical hash at a REALISTIC batch size. See CODING_GUIDELINES.md 9.2.
