...
class SupportField:
    ...
    def _func_support_prism(self, d, i_g, i_b):
        ...
        ibest = istart
        best = self.solver.collider._collider_state.prism[istart, i_b].dot(d)
        for i in range(istart + 1, istart + 3):
            dot = self.solver.collider._collider_state.prism[i, i_b].dot(d)
            if dot > best:
                ibest = i
                best = dot

        return self.solver.collider._collider_state.prism[ibest, i_b], ibest
