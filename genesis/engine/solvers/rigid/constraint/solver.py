...
def func_solve_init(
    ...
    if qd.static(
        ...
        # Monolith arm only. The decomposed Newton arm computes its first direction itself: its graph is
        # direction-first, so iteration 1 factors from the state this kernel set up (qacc/Ma/Jaref/forces/qfrc/cost) -
        # no separate seed factor needed here. The monolith is linesearch-first and DOES need the seed, except for the
        # GPU island case where it self-inits the factor per-env in its own body; so it is done here for the monolith
        # on non-island (any backend) and CPU-island Newton.
    ...
    if qd.static(
        ...
        # Initial gradient (Mgrad = H^-1 grad for Newton, grad for CG). Skipped for the decomposed Newton arm (its
        # direction-first graph computes the gradient itself on iteration 1) and the GPU island monolith (self-inits).
        # Still runs for the CG decomposed arm, whose first save_prev_grad needs this seed gradient.
