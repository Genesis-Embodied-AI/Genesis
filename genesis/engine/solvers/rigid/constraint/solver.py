...
def add_collision_constraints(
    ...
    if qd.static(static_rigid_sim_config.constraint_layout_transposed):
        ...
        qd.loop_config(
            name="add_collision_constraints", serialize=static_rigid_sim_config.para_level < gs.PARA_LEVEL.ALL
        )
        ...
        # Legacy threading: bit-identical to the pre-flip baseline. 1 thread per (i_col, i_b), each
        # thread processes all 4 friction bases inline so per-(i_col, i_b) setup (contact_data reads,
        # d1/d2 = orthogonals(normal), invweight) is hoisted out of the for-i loop and shared across
        # the 4 frictions. Adjacent lanes vary i_b (flat_idx % _B), coalesced under canonical
        # (n_constraints, n_dofs, _B). Inlined rather than calling the helper because the CPU backend
        # doesn't always inline qd.func cleanly and the helper's per-call setup causes a small but
        # consistent regression on small CPU workloads (e.g. anymal_uniform_kinematic, -7.9% pre-fix).
        EPS = rigid_global_info.EPS[None]
        n_dofs = dofs_state.ctrl_mode.shape[0]
        qd.loop_config(
            name="add_collision_constraints", serialize=static_rigid_sim_config.para_level < gs.PARA_LEVEL.ALL
