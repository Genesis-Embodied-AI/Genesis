    ...
    def import_from_stage(
        ...
        for articulation_root in articulation_roots:
            morph = gs.morphs.USDArticulation(
                ...
                convexify=usd_morph.convexify,
                decompose_object_error_threshold=usd_morph.decompose_object_error_threshold,
                decompose_robot_error_threshold=usd_morph.decompose_robot_error_threshold,
                coacd_options=usd_morph.coacd_options,
                decimate=usd_morph.decimate,
                decimate_face_num=usd_morph.decimate_face_num,
                decimate_aggressiveness=usd_morph.decimate_aggressiveness,
            )
...
def import_from_usd(
    scene: gs.Scene, morph: USDMorph, vis_mode: Literal["visual", "collision"], visualize_contact: bool = False
):
    stage = Usd.Stage.Open(morph.file)
    return UsdParser.import_from_stage(scene, stage, vis_mode, morph, visualize_contact)
