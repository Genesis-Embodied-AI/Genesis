...
# Compat alias for V_ANNOTATION (used as a kernel parameter type hint by a few of our
# AMD-tuned kernels in `genesis/engine/solvers/rigid/...`). At runtime this resolves to
# `qd.template` (Quadrants compile-time template parameter) when running with the field
# backend, and to `qd.types.ndarray()` (runtime ndarray parameter) when running with the
# ndarray backend - matching the previous behavior from the amd-integration branch before
# upstream's v1.0.0 refactor removed it from this module.
if TYPE_CHECKING:
    V_ANNOTATION = qd.Field | qd.Ndarray  # type: ignore[name-defined,operator]
else:
...
class ConstraintState:
    ...
    ls_gtol: qd.Tensor
    # Compacted list of active-constraint candidates for the line search batched-update path.
    candidates: qd.Tensor
    eq_sum: qd.Tensor
    ls_it: qd.Tensor
    ls_result: qd.Tensor
    # Optional CG fields
...
class ColliderState:
    ...
    grid_cell_key: qd.Tensor          # hash of grid cell for each geom (center-based)
    grid_cell_count: qd.Tensor        # count of geoms per cell
    grid_cell_start: qd.Tensor        # start index in sorted array per cell
    grid_sorted_geom_id: qd.Tensor    # geom indices sorted by cell
    grid_scatter_idx: qd.Tensor       # temp index for atomic scatter
    grid_total_entries: qd.Tensor     # total number of entries
...
class GeomsState:
    ...
    active_buffer_idx: qd.Tensor  # Index in active_buffer for O(1) removal
