...
def func_planner_collision_cost(
            ...
            for i_gw in range(plan_world.n_geoms[None]):
                if plan_world.geoms_is_active[i_gw, i_b] and not func_planner_world_aabb_skip(
                    ...
                    sd_eff = (
                        ...
                        - swp
                        - func_planner_excl_world_offset(i_s, i_gw, i_b, plan_info)
                    )
    ...
    for i_p in range(plan_info.self_pairs.shape[0]):
        ...
        if radius_a > 0.0 and radius_b > 0.0:
            sd_eff = (
                (spheres_pos[i_sa, i_col] - spheres_pos[i_sb, i_col]).norm()
                - radius_a
                - radius_b
                - plan_info.d_safe[None]
                - 2.0 * swp
                - func_planner_excl_self_offset(i_p, i_b, plan_info)
            )
            min_sd = qd.min(min_sd, sd_eff)
...
def kernel_planner_cost_grad(
    ...
    """
    Full cost and joint-space gradient of every active (candidate, knot) column, from the FK cache written by
    kernel_planner_fk_cache. One thread owns one column and writes only its own cost_wp / grad_traj slices - no
    atomics, deterministic per backend. The sweep allowance is stop-gradient (see the design plan): it gates where
    the hinge activates without contributing cross-column gradient terms.
    """
    ...
    for i_cw in range(envs_idx.shape[0] * n_seeds * n_knots):
        ...
        if plan_state.is_active[i_c]:
            ...
            for i_s in range(n_sph_tot):
                if i_w > 0:
                    swp = qd.max(
                        swp, 0.5 * (plan_state.spheres_pos[i_s, i_cw] - plan_state.spheres_pos[i_s, i_cw - 1]).norm()
                    )
                if i_w < n_knots - 1:
                    swp = qd.max(
                        swp, 0.5 * (plan_state.spheres_pos[i_s, i_cw + 1] - plan_state.spheres_pos[i_s, i_cw]).norm()
                    )
            ...
            if plan_info.has_pose_goal[None] and i_w == n_knots - 1:
                ...
                while i_l_glob != -1:
                    I_l = (
                        [gs.qd_int(i_l_glob), i_b] if qd.static(rigid_config.batch_links_info) else gs.qd_int(i_l_glob)
                    )
                    if dyn_info.links.parent_idx[I_l] == -1 and dyn_info.links.is_fixed[I_l]:
                        i_l_glob = -1
                    else:
                            ...
                            if dyn_info.joints.type[I_j] == gs.JOINT_TYPE.REVOLUTE:
                                ...
                                if not plan_info.dof_is_locked[i_dp, i_b]:
                                    plan_state.grad_traj[i_dp, i_cw] += (
                                        ...
                                        * plan_state.joints_xaxis[
                                            i_j - qd.static(planner_config.joint_offset), i_cw
                                        ].dot(err_rot)
                                    )
                        i_l_glob = dyn_info.links.parent_idx[I_l]
