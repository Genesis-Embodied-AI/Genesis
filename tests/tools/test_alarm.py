...
def run_alarm_script(artifacts_dir: Path, output_dir: Path, wandb_api_mock: MockWandbApi, 
                     alarm_yml_path: Path):
    """
...
Test Scenarios:
  ...
  new_benchmark     - New benchmark not in historical data
    ...
    parser.add_argument(
        "--temp-dir",
        type=Path,
        help="Use specific directory for temporary files (implies --keep-temp)"
    )
    parser.add_argument(
        "--alarm-yml",
        type=Path,
        help="Path to alarm.yml file (default: auto-detect from script location)"
    )
