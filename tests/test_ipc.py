...
def test_ipc_rigid_ground_clearance(n_envs, show_viewer):
    ...
    # so d scales linearly with kappa (and ~linearly with R_cube when R_cube << R_plane). As R_cube approaches
    # R_plane the kappa harmonic mean saturates and the cube exits the deep-barrier regime — the d-ratio drops
    # below the kappa-ratio. With rho=200 cubes, only the first two ratios (R=1e2/1e3, 1e3/1e4) remain in the
    # deep regime; (1e4/1e5) is near saturate; (1e5/1e6) is near-saturate-to-saturate.
    deep_barrier_diff_ratios = dist[..., 1:-2, :] / dist[..., :-3, :]
    assert (deep_barrier_diff_ratios > 9).all()

    near_saturate_diff_ratios = dist[..., -2, :] / dist[..., -3, :]
    assert (near_saturate_diff_ratios > 3).all()

    near_saturate_to_saturate_diff_ratios = dist[..., -1, :] / dist[..., -2, :]
    assert (near_saturate_to_saturate_diff_ratios > 1.1).all()
...
def test_cloth_uniform_biaxial_stretching(E, nu, strech_scale, n_envs, show_viewer):
        ...
        for z_sign in (+1, -1):
            box = scene.add_entity(
                ...
                material=gs.materials.Rigid(
                    ...
                    soft_constraint_strength=(2000.0, 100.0),
                ),
                surface=gs.surfaces.Plastic(
    ...
    assert_allclose(cloth_positions_f[..., 2], cloth_positions_0[..., 2], tol=0.005)
...
def test_ext_art_coupler_fixed_merge():
    ...
    assert find_target_link_for_fixed_merge(hand) is link7
