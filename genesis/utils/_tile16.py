...
def _make_tile16x16_class(dtype):
    class _Tile16x16Cholesky:
        ...
        def cholesky_(self, eps):
            ...
            # compile time and `_get_col(k)` collapses to a single register access
            # rather than a 16-deep register-indexing cascade.
    ...
    def _resolve(dtype):
        ...
        if arch in (qd.cpu, qd.x64, getattr(qd, "arm64", None)):
            raise QuadrantsSyntaxError(
                "Tile16x16Cholesky requires a GPU backend (cuda, metal, vulkan, amdgpu). " f"Current arch is {arch}."
            )
        if dtype is None:
