...
class IPCCoupler(RBC):
    ...
    def __init__(self, simulator: "Simulator", options: IPCCouplerOptions) -> None:
        ...
        self._ipc_coup_types: dict["RigidEntity", COUPLING_TYPE] = {}
        ...
        self._coupling_entries: list[tuple[int, "RigidLink", int]] = []
        self._coupling_data: IPCCouplingData | None = None

        # ==== GUI ====
        self._ipc_gui: SceneGUI | None = None  # polyscope viewer, only when show_ipc_gui=True
    ...
    def _setup_coupling_config(self):
        """Read ipc_coup_type, coup_links, and collision settings from entity materials."""
        ...
        for i_e, entity in enumerate(cast(list["RigidEntity"], self.rigid_solver.entities)):
            if not entity.material.needs_coup:
                continue
            ipc_coup_type = entity.material.ipc_coup_type
            if ipc_coup_type is None:
                continue
            ...
            if not entity.material.enable_ipc_collision:
            ...
            elif entity.material.ipc_collision_links is not None:
                # Positive filter: only named links get collision, others disabled
                allowed = set(entity.material.ipc_collision_links)
        ...
        for entity, ipc_coup_type in self._ipc_coup_types.items():
            self._entities_by_coup_type.setdefault(ipc_coup_type, []).append(entity)
    ...
    def _add_rigid_geoms_to_ipc(self) -> None:
        ...
        gs.logger.debug(f"Registered entity coupling types: {set(self._ipc_coup_types.values())}")

        # Create constitutions based on coupling types present
        coup_types_present = set(self._ipc_coup_types.values())
        ...
        for link in self.rigid_solver.links:
            ...
            ipc_coup_type = self._ipc_coup_types.get(entity)
            if ipc_coup_type is None:
                continue

            # Link filter for two_way_soft_constraint
            if ipc_coup_type == COUPLING_TYPE.TWO_WAY_SOFT_CONSTRAINT:
        ...
        for env_idx in range(self.sim._B):
            for target_link, source_links in target_groups.items():
                ...
                entity_ipc_coup_type = self._ipc_coup_types[entity]
                    ...
                    for geom in source_link.geoms:
                        if geom.type == gs.GEOM_TYPE.PLANE:
                            if entity_ipc_coup_type != COUPLING_TYPE.IPC_ONLY:
                                gs.raise_exception(
                                    f"Plane entity (solver idx={i_e}) has ipc_coup_type='{entity_ipc_coup_type}', "
                ...
                abd_geom = meshes[0] if len(meshes) == 1 else uipc.geometry.merge(meshes)
                uipc.geometry.label_surface(abd_geom)

                trans_T = gu.trans_quat_to_T(links_pos[env_idx, target_link.idx], links_quat[env_idx, target_link.idx])
                trans_view = uipc.view(abd_geom.transforms())
                trans_view[0] = trans_T

                # ---- Determine coupling behavior ----
                is_ipc_only = entity_ipc_coup_type == COUPLING_TYPE.IPC_ONLY
                is_free_base = (
                    entity_ipc_coup_type == COUPLING_TYPE.EXTERNAL_ARTICULATION
                    and target_link is entity.base_link
                    and not entity.base_link.is_fixed
                )
                is_soft_constraint_target = entity_ipc_coup_type == COUPLING_TYPE.TWO_WAY_SOFT_CONSTRAINT or (
                ...
                if self.sim.n_envs > 0:
                    self._ipc_subscenes[env_idx].apply_to(abd_geom)
                ...
                if self._coupling_collision_settings.get(entity, {}).get(target_link, True):
                    ...
                    self._ipc_abd_contacts[entity].apply_to(abd_geom)
                else:
                    self._ipc_no_collision_contact.apply_to(abd_geom)

                self._ipc_abd.apply_to(abd_geom, kappa=ABD_KAPPA * uipc.unit.MPa, mass_density=entity.material.rho)
                ...
                if is_soft_constraint_target:
                    ...
                    self._ipc_stc.apply_to(abd_geom, constraint_strength)
                ...
                external_kinetic_attr = abd_geom.instances().find(uipc.builtin.external_kinetic)
                uipc.view(external_kinetic_attr)[:] = int(not is_free_base_ipc_driven and not is_ipc_only)

                is_fixed_attr = abd_geom.instances().find(uipc.builtin.is_fixed)
                ...
                if entity_ipc_coup_type == COUPLING_TYPE.EXTERNAL_ARTICULATION and self.options.enable_rigid_dofs_sync:
                    ref_dof_prev_attr = abd_geom.instances().create("ref_dof_prev", uipc.Vector12.Zero())
                    uipc.view(ref_dof_prev_attr)[:] = uipc.geometry.affine_body.transform_to_q(trans_T)

                # set metadata attributes
                meta_attrs = abd_geom.meta()
                meta_attrs.create("solver_type", "rigid")
                meta_attrs.create("link_idx", str(target_link.idx))
                meta_attrs.create("env_idx", str(env_idx))

                # ---- Create IPC object and geometry slot ----
                abd_geom_slot, _ = abd_obj.geometries().create(abd_geom)
    ...
    def _add_articulation_entities_to_ipc(self) -> None:
        ...
        if COUPLING_TYPE.EXTERNAL_ARTICULATION not in self._ipc_coup_types.values():
            return
        ...
        for i_e, entity in enumerate(cast(list["RigidEntity"], self.rigid_solver.entities)):
            ...
            if self._ipc_coup_types.get(entity) != COUPLING_TYPE.EXTERNAL_ARTICULATION:
                continue
            ...
            for joint in entity.joints[(0 if entity.base_link.is_fixed else 1) :]:
                if joint.type == gs.JOINT_TYPE.FIXED:
                    ...
                    constitution_cls = AffineBodyRevoluteJoint
                    reverse_verts = True
                elif joint.type == gs.constants.JOINT_TYPE.PRISMATIC:
                    constitution_cls = AffineBodyPrismaticJoint
                    reverse_verts = False
                else:
                ...
                joints.append((joint, constitution_cls, reverse_verts, parent_link, child_link))

            # ---- Create joint geometries per environment ----
            articulation_geom_slots: list[GeometrySlot] = []
            for env_idx in range(self.sim._B):
                joint_geom_slots: list[GeometrySlot] = []
                for joint, constitution_cls, reverse_verts, parent_link, child_link in joints:
                    ...
                    constitution_cls().apply_to(joint_geom, [parent_abd_slot], [0], [child_abd_slot], [0], [100.0])
                ...
                articulation_geom = self._ipc_eac.create_geometry(joint_geom_slots, [0 for _ in joint_geom_slots])
    ...
    def _init_accessors(self):
        ...
        for link, _ in self._abd_primary_links:
            self._abd_transforms_by_link.setdefault(link, []).append(np.eye(4, dtype=gs.np_float))
        ...
        self._coupling_entries = [
            (abd_body_idx, link, env_idx)
            for abd_body_idx, (link, env_idx) in enumerate(self._abd_primary_links)
            if link.entity in rigid_retrieve_entities
        ]
        self._coupling_data = IPCCouplingData(self._coupling_entries)

        gs.logger.debug(f"IPC coupling index mapping created: {len(self._coupling_entries)} entries.")
    ...
    def has_any_rigid_coupling(self) -> bool:
        ...
        """
        return bool(self._ipc_coup_types)
    ...
    def _retrieve_fem_states(self):
        ...
        for fem_geom_slot in visitor.geometries():
            ...
            merged_geom = fem_geom
            if fem_geom.instances().size() >= 1:
                merged_geom = uipc.geometry.merge(uipc.geometry.apply_transform(fem_geom))
            merged_geom_positions = merged_geom.positions().view().reshape(-1, 3)
            fem_positions_by_entity[entity][env_idx] = merged_geom_positions
    ...
    def _retrieve_rigid_states(self):
        ...
        for abd_body_idx, link, env_idx in self._coupling_entries:
            aim_transform = self._abd_transforms_by_link[link][env_idx]
            ...
            link_idx_local = self._coupling_data.idx_by_link[link]
            self._coupling_data.ipc_transforms[env_idx, link_idx_local] = transform_matrix
            self._coupling_data.aim_transforms[env_idx, link_idx_local] = aim_transform
    ...
    def _store_gs_rigid_states(self):
        ...
        for link, env_idx in self._abd_primary_links:
            self._abd_transforms_by_link[link][env_idx][:] = links_transform[env_idx, link.idx]
