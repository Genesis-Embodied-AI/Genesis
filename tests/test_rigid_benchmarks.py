...
@pytest.fixture
def anymal_regiment(solver, n_envs, gjk, pytorch_profiler_step):
    scene = gs.Scene(
        rigid_options=gs.options.RigidOptions(
            **get_rigid_solver_options(
                dt=STEP_DT,
    ...
    while True:
        robot.control_dofs_position(
            torch.rand((12,), dtype=gs.tc_float, device=gs.device) * 0.1 - 0.05, motors_dof_idx
        )
        scene.step()
        pytorch_profiler_step()
        time_elapsed = time.time() - time_start
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("anymal_regiment", None, None, 30000, gs.gpu),
