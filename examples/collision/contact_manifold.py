...
def main():
    ...
    parser.add_argument("-s", "--steps", type=int, default=LOOP_STEPS, help="Number of simulation steps")
    parser.add_argument(
        "--contact-patch",
        action="store_true",
        help="Recover the manifold from the contact patch instead of perturbed re-detections",
    )
    ...
    scene = gs.Scene(
        rigid_options=gs.options.RigidOptions(
            ...
            enable_contact_patch=args.contact_patch,
        ),
    ...
    for i in range(args.steps):
        # One continuous loop: edge tilt, yaw, vertical interpenetration, lateral slide, then unwind back to the start.
        t = (i % LOOP_STEPS) / LOOP_STEPS
        yaw, tilt, slide, penetration = MAX_YAW, 0.0, 0.0, PENETRATION
        if t < 0.20:
            # The tilt is taken at yaw 0, where the rocking edge stays over the fixed box's top face. At yaw 45 the
            # deepest corner overhangs that face, so the pair would rest on an edge of the fixed box instead.
        ...
        # Hold the deepest corner exactly `penetration` below the fixed box's top face, whatever the orientation is.
        ...
        if len(positions) != n_contacts:
            ...
            print(
                f"step {i:5d}  yaw {yaw:5.1f} deg  tilt {tilt:4.1f} deg  penetration {1e3 * penetration:5.2f} mm  "
                f"manifold {n_contacts} points"
            )
        if args.vis:
