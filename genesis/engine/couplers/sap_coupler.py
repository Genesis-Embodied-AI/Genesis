...
class SAPCoupler(RBC):
    ...
    def _init_hydroelastic_rigid_fields_and_info(self):
        ...
        collider_info = self.rigid_solver.collider._collider_info
        np_valid_pairs_a = collider_info.valid_pairs_a.to_numpy()
        np_valid_pairs_b = collider_info.valid_pairs_b.to_numpy()
        n_vp = int(collider_info.n_valid_pairs.to_numpy())
        self.rigid_valid_pairs_a = qd.field(gs.qd_int, shape=np_valid_pairs_a.shape)
        self.rigid_valid_pairs_a.from_numpy(np_valid_pairs_a)
        self.rigid_valid_pairs_b = qd.field(gs.qd_int, shape=np_valid_pairs_b.shape)
        self.rigid_valid_pairs_b.from_numpy(np_valid_pairs_b)
        self.rigid_n_valid_pairs = qd.field(gs.qd_int, shape=())
        self.rigid_n_valid_pairs.fill(n_vp)
