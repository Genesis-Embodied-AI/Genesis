...
@pytest.mark.required
@pytest.mark.parametrize("backend", [gs.cpu, gs.gpu])
def test_inverse_kinematics_local_point(show_viewer):
    ...
    # Target position where we want the offset point to be
    target_pos = torch.tensor([0.5, 0.2, 0.4], dtype=gs.tc_float, device=gs.device)
    target_quat = torch.tensor([0.0, 1.0, 0.0, 0.0], dtype=gs.tc_float, device=gs.device)  # Pointing down

    # Solve IK with local_point
