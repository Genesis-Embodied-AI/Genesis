...
class Raytracer:
    def __init__(self, options, vis_options):
        ...
        for light in options.lights:
            ...
            # LuisaRender produces NaN when an emissive mesh is coincident with scene geometry.
            # Ensure the sphere stays above the ground plane (z=0) by a small margin.
            pos = np.array(light.pos, dtype=float)
            pos[2] = max(pos[2], light.radius + 1e-3)
            self.lights.append(SphereLight(radius=light.radius, pos=pos, surface=light_surface))
