...
class RigidSimStaticConfig(metaclass=AutoInitMeta):
    ...
    # ``func_cholesky_and_solve_fused_tiled`` (single kernel, L kept in shmem) instead of the separate
    # ``func_cholesky_factor_direct_tiled`` + ``func_cholesky_solve_tiled`` pair. Saves the per-step global-mem L
    # roundtrip between factor and solve. Microbench at dex_hand scale (4096 envs * 64 dofs, f32) shows ~1.25x speedup
    # on the cholesky work; that's ~1.2-1.7% FPS on dex_hand where cholesky is ~6.4% of step time. Requires
