...
def main():
    ...
    gripper = scene.add_entity(
        morph=gs.morphs.URDF(
            file="urdf/simple_band_gripper/band_gripper.urdf",
    ...
    for frame in range(total_frames):
        ...
        if total_t < grip_duration:
            ...
            left_top_target = np.array([
                0.0, left_y, current_z_top,
                zero_rotation[0], zero_rotation[1], zero_rotation[2]
            ])
            
            left_bottom_target = np.array([
                0.0, left_y, current_z_bottom,
                zero_rotation[0], zero_rotation[1], zero_rotation[2]
            ])
            
            # Right end grippers (at +Y)
            right_top_target = np.array([
                0.0, right_y, current_z_top,
                zero_rotation[0], zero_rotation[1], zero_rotation[2]
            ])
            
            right_bottom_target = np.array([
                0.0, right_y, current_z_bottom,
                zero_rotation[0], zero_rotation[1], zero_rotation[2]
            ])
        ...
        # Phase 2: Twisting - rotate grippers in opposite directions around Y axis
            ...
            left_top_target = np.array([
                left_top_x, left_y, left_top_z,
                0.0, angle, 0.0  # rx, ry, rz - rotate around Y
            ])
            
            left_bottom_target = np.array([
                left_bottom_x, left_y, left_bottom_z,
                0.0, angle, 0.0
            ])
            ...
            right_top_target = np.array([
                right_top_x, right_y, right_top_z,
                0.0, -angle, 0.0
            ])
            
            right_bottom_target = np.array([
                right_bottom_x, right_y, right_bottom_z,
                0.0, -angle, 0.0
            ])
        ...
        # Progress reporting
        if frame % 100 == 0:
            phase = "Gripping" if total_t < grip_duration else \
                   "Twisting" if total_t < twist_start_time + twist_duration else \
                   "Settling"
