...
def fill_orthographic(dense):
    shape = dense.shape
    indices = np.stack(np.meshgrid(*(np.arange(s) for s in shape), indexing="ij"), axis=-1)
...
def matrix_to_marching_cubes(matrix, pitch=1.0):
    ...
    pad_width = 1
    rev_matrix = np.pad(rev_matrix, pad_width=(pad_width), mode="constant", constant_values=(1))
    ...
    if pitch.size == 1:
        pitch = (pitch,) * 3
    meshed = func(volume=rev_matrix, level=0.5, spacing=pitch)  # it is a boolean voxel grid
...
def multibox(centers, pitch=1.0, colors=None):
    ...
    f += np.tile(np.arange(len(centers)) * len(b.vertices), (len(b.faces), 1)).T.reshape((-1, 1))
