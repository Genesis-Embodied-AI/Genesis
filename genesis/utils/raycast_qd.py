...
def update_aabbs(
    ...
    AABB slot k holds the bounding box of face `face_ids[k]` when `is_remapped` (a compile-time flag, see
    bvh_ray_cast), so the rebuild scales with the subset size; otherwise slot k holds face k and `face_ids` is
    ignored.
