...
- **Pass an anonymous literal by keyword** so its meaning shows at the call site (`_adaptive_params(verts, faces, aggressiveness=7)`, not a bare `7`). Positional is fine for self-named variables. This holds in rigid kernel and func calls too: anonymous constants (and everything after them) go by keyword; only self-named arguments are positional.
- **Line wrapping is all-or-nothing, per nesting level.** A call either fits on one line at its indent (<= 120 chars - count before exploding) or takes one argument per line; never 2-3 arguments per continuation line. Inside an exploded outer call, a nested call that fits stays on ONE line (`shape=maybe_shape((_B, solver.n_dofs_), solver._rigid_config.enable_cone_free_hessian_reuse)` inside an exploded `V(...)`). ruff never collapses an exploded call (magic trailing comma), so write the compact form by hand. Matrix literals (lists of lists) are the exception: keep one row per line even when the whole literal fits on one.
