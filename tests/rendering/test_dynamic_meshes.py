...
def test_deformable_uv_textures(renderer_type, renderer, show_viewer, png_snapshot, backend):
    ...
    # Quadrants 1.1.3b1 enables native Metal f32 atomic_add (avoids the CAS-path MRE), but this full FEM+PBD
    # workload (n_pcg_iterations=40) still wedges on macos-15 CI - keep the skip until that is fixed.
