...
from .kinematics import func_forward_kinematics, func_inverse_kinematics_jacobian, func_sphere_positions
from .world import func_world_aabb_skip, func_world_signed_distance, func_world_signed_distance_gradient
...
@qd.func
def func_exclusion_world_offset(i_s, i_gw, i_bound, i_b, planner_info: array_class.PlannerEntityInfo):
    ...
    Looked up through the (sphere, boundary) key bracket, so the walk covers the geoms that one sphere is excluded
    against instead of every entry of the env - every (sphere, geom) pair reaching the distance query asks for one,
    which makes a walk proportional to the list the dominant term of the obstacle loop (see world_range in
    array_class.py).
    """
    offset = gs.qd_float(0.0)
    i_x_range = planner_info.cert.excl.world_range[i_s, i_bound, i_b]
    for i_x in range(i_x_range[0], i_x_range[1]):
        if planner_info.cert.excl.world_geom[i_x, i_b] == i_gw:
...
@qd.func
def func_exclusion_world_anchor(i_s, i_gw, i_bound, i_b, planner_info: array_class.PlannerEntityInfo):
...
@qd.func
def func_exclusion_self_anchor(i_p, i_bound, i_b, planner_info: array_class.PlannerEntityInfo):
    """Anchor relative offset of an excluded self-collision sphere pair (see func_exclusion_world_anchor)."""
    anchor = qd.Vector([0.0, 0.0, 0.0], dt=gs.qd_float)
...
def func_gjk_clearance(
    ...
    if dist <= collider_info.gjk.collision_eps[None]:
        ...
        if is_swept_a:
            dist = dist - dyn_info.geoms.data[i_ga][0]
        if is_swept_b:
            dist = dist - dyn_info.geoms.data[i_gb][0]
    return dist
            ...
            for i_j_ in range(dyn_info.links.joint_start[I_l], dyn_info.links.joint_end[I_l]):
                ...
                if joint_type != gs.JOINT_TYPE.FIXED:
                    ...
                    if not planner_info.fk.dofs.is_locked[i_dp, i_b]:
                        ...
                        grad_traj[i_dp, i_c, i_w] = grad_traj[i_dp, i_c, i_w] + (scale * dq)
    ...
    for i_lp in range(planner_info.fk.self_pairs.link_pairs_idx.shape[0]):
        ...
        for i_p in range(i_p_start, i_p_end):
            ...
            if radius_a > 0.0 and radius_b > 0.0:
                ...
                for i_bound in range(2):
                    offset = func_exclusion_self_offset(i_p, i_bound, i_b, planner_info)
                    if offset < 0.0:
                        # See the world-pair exclusion above.
                        anchor = func_exclusion_self_anchor(i_p, i_bound, i_b, planner_info)
                ...
                hinge, _ = func_hinge_cost(sd_eff, planner_info.cost.eps_self[None])
...
def func_merge_boundary_exclusions(
    ...
    """
    n_spheres_with_attach = qd.static(planner_config.n_spheres + planner_config.n_attach_max)
    n_excl_max = planner_info.cert.excl.world_geom.shape[0]
                    ...
                    if sd_eff < 0.0 and raw < contact_band and raw > -depth_max:
                        ...
                        if not is_listed:
                            if n_world < n_excl_max:
                                planner_info.cert.excl.world_geom[n_world, i_b] = i_gw
                                ...
                                # Appending in key order - boundary outermost, then sphere, then geom - is what
                                # keeps a key's entries contiguous, so one bracket addresses them. An empty key
                                # starts where it is appended, one already holding entries only extends.
                                i_x_first = n_world if i_x_range[1] == i_x_range[0] else i_x_range[0]
                                planner_info.cert.excl.world_range[i_s, i_bound, i_b] = qd.Vector(
                                    [i_x_first, n_world + 1], dt=gs.qd_int
                                )
...
def func_boundary_exclusions(
    ...
    for i_b_ in range(envs_idx.shape[0]):
        ...
        for i_s in range(qd.static(planner_config.n_spheres + planner_config.n_attach_max)):
            for i_bound in range(2):
                planner_info.cert.excl.world_range[i_s, i_bound, i_b] = qd.Vector([0, 0], dt=gs.qd_int)
        for i_p in range(planner_info.fk.self_pairs.spheres_idx.shape[0]):
            for i_bound in range(2):
        ...
        for i_bound in range(qd.static(2 if include_goal else 1)):
            ...
            is_overflow = is_overflow | func_merge_boundary_exclusions(
                i_b_,
                i_b,
                i_bound,
                contact_band,
...
def func_knot_cost_gradient(
    ...
    for i_dp in range(n_dp):
        if i_w > 0:
            dq_inf = qd.max(
                dq_inf, qd.abs(planner_state.cost.qpos[i_dp, i_c, i_w] - planner_state.cost.qpos[i_dp, i_c, i_w - 1])
            )
        if i_w < n_knots - 1:
            dq_inf = qd.max(
                dq_inf, qd.abs(planner_state.cost.qpos[i_dp, i_c, i_w + 1] - planner_state.cost.qpos[i_dp, i_c, i_w])
            )
                ...
                if planner_world.geoms_is_active[i_gw, i_b] and not func_world_aabb_skip(
                    ...
                    for i_bound in range(2):
                        ...
                        if offset < 0.0:
                            ...
                            anchor, _ = func_exclusion_world_anchor(i_s, i_gw, i_bound, i_b, planner_info)
                            drift = x - anchor
                    ...
                    hinge, dhinge = func_hinge_cost(sd_eff, eps_act)
                    if hinge > 0.0:
                        cost = cost + (planner_info.cost.w_obs[None] * hinge)
                        normal = func_world_signed_distance_gradient(
            ...
            for i_bound in range(2):
                ...
                if offset < 0.0:
                    ...
                    if sd_excused > sd_eff:
                        ...
                        if ramp > 0.0:
                            ramp_dir = drift / drift_norm
            hinge, dhinge = func_hinge_cost(sd_eff, planner_info.cost.eps_self[None])
    ...
    # Smoothness (acceleration + jerk finite-difference quadratics) and joint-limit hinge + posture regularizer,
    # all local stencils on the candidate's own knots.
    for i_dp in range(n_dp):
        if not planner_info.fk.dofs.is_locked[i_dp, i_b]:
            ...
            for dw in qd.static(range(-1, 2)):
                ...
                if 1 <= i_wc < n_knots - 1:
                    acc = (
                        planner_state.cost.qpos[i_dp, i_c, i_wc - 1]
                        - 2.0 * planner_state.cost.qpos[i_dp, i_c, i_wc]
                        + planner_state.cost.qpos[i_dp, i_c, i_wc + 1]
                    )
                    coeff = gs.qd_float(-2.0 if dw == 0 else 1.0)
                    if dw == 0:
                        cost = cost + (planner_info.cost.w_acc[None] * acc**2)
                    planner_state.cost.grad[i_dp, i_c, i_w] = planner_state.cost.grad[i_dp, i_c, i_w] + (
                        2.0 * planner_info.cost.w_acc[None] * acc * coeff
                    )
            ...
            for dw in qd.static(range(-1, 3)):
                ...
                if 1 <= i_wc < n_knots - 2:
                    jerk = (
                        planner_state.cost.qpos[i_dp, i_c, i_wc + 2]
                        - 3.0 * planner_state.cost.qpos[i_dp, i_c, i_wc + 1]
                        + 3.0 * planner_state.cost.qpos[i_dp, i_c, i_wc]
                        - planner_state.cost.qpos[i_dp, i_c, i_wc - 1]
                    )
                    ...
                    planner_state.cost.grad[i_dp, i_c, i_w] = planner_state.cost.grad[i_dp, i_c, i_w] + (
                        2.0 * planner_info.cost.w_jerk[None] * jerk * coeff
                    )
            ...
            if over > 0.0:
                ...
                planner_state.cost.grad[i_dp, i_c, i_w] = planner_state.cost.grad[i_dp, i_c, i_w] + (
                    2.0 * planner_info.cost.w_lim[None] * over
                )
            if under > 0.0:
                cost = cost + (planner_info.cost.w_lim[None] * under**2)
                planner_state.cost.grad[i_dp, i_c, i_w] = planner_state.cost.grad[i_dp, i_c, i_w] - (
                    2.0 * planner_info.cost.w_lim[None] * under
                )
            ...
            planner_state.cost.grad[i_dp, i_c, i_w] = planner_state.cost.grad[i_dp, i_c, i_w] + (
                2.0 * planner_info.cost.w_posture[None] * (q - ref)
            )
    ...
    if planner_info.cost.boundary.has_pose_goal[None] and i_w == n_knots - 1:
        ...
        err_rot = gu.qd_quat_to_rotvec(quat_rel, rigid_info.EPS[None])
        cost = cost + (planner_info.cost.w_pose_pos[None] * err_pos.norm_sqr())
        cost = cost + (planner_info.cost.w_pose_rot[None] * err_rot.norm_sqr())
    ...
    for i_cl in range(envs_idx.shape[0] * n_seeds * n_lanes):
        ...
        if True:
            ...
            for i_sample_chunk in range(n_sample_chunks):
                ...
                dq_inf = gs.qd_float(0.0)
                for i_dp in range(n_dp):
                    q0 = planner_state.cost.qpos[i_dp, i_c, i_w]
                    q1 = planner_state.cost.qpos[i_dp, i_c, i_w + 1]
                    q = q0 + alpha * (q1 - q0)
                    planner_state.fk.eval.qpos[i_dp, i_e_col] = q
                    # Half the inter-sample joint travel bounds the workspace motion of every sphere.
                    swp = swp + (0.5 * planner_info.fk.dofs.reach[i_dp] * qd.abs(q1 - q0) / float(n_upsample))
            ...
            if qd.static(check_start):
                for i_dp in range(n_dp):
                    if (
                        qd.abs(planner_state.cost.qpos[i_dp, i_c, 0] - planner_info.cost.boundary.qpos_start[i_dp, i_b])
                        > 1e-4
                    ):
            ...
            if qd.static(n_lanes > 1):
                ...
                for i_bit in qd.static(
                    ...
                    flags_union = flags_union + qd.simt.subgroup.reduce_all_max_tiled(flags & i_bit, log2_lanes)
                flags = flags_union
            ...
            if planner_info.cost.boundary.has_pose_goal[None]:
                ...
                for i_dp in range(n_dp):
                    q_end = planner_state.cost.qpos[i_dp, i_c, n_knots - 1]
        ...
        for i_c in range(envs_idx.shape[0] * n_restarts):
            ...
            if not planner_state.is_env_solved[i_b] and not planner_state.is_goal_free[i_b]:
                ...
                for _ in range(qd.static(planner_config.goal_ik_iters)):
                    ...
                    func_inverse_kinematics_jacobian(
                        i_c,
                        i_b,
                        goal_link,
                        ee_pos,
                    ...
                    for i_dp in range(n_dp):
                        if not planner_info.fk.dofs.is_locked[i_dp, i_b]:
                            ...
                            for j in range(6):
                                dq = dq + (
                                    planner_state.ik.jacobian_stacked_t[i_dp, j, i_c] * planner_state.ik.vec[j, i_c]
                                )
