...
- **In docstrings, a single pair of backticks marks anything that names a Python object, a double pair marks everything else; quotes never mark code.** Single backticks carry the `py:obj` role, so a resolvable name links to its entry and any other renders as inline code. A bare name resolves only within the class owning it (an enum member in its own enum docstring); a cross-module reference takes the real dotted path prefixed with `~` (`` `~genesis.constants.integrator` ``), and the `gs.*` aliases and `Options` fields never resolve, so write them as they should read. Values and code fragments take a double pair: ` ``"srgb"`` `, ` ``True`` `, ` ``metal_type="iron"`` `.
...
- **Be concise, but do not write fragments.** Here is a clear anti-example: "They differ in what else is folded in, and when." A reader should never have to reconstruct an elided clause to understand what is written.
