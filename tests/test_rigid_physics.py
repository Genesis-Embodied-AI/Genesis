...
def test_path_planning_avoidance(show_viewer):
    ...
    for path, ignore_collision in ((free_path, False), (avoidance_path, True)):
        ...
        for waypoint in path:
            ...
            for i in range(scene.rigid_solver.collider.collider_global_info.n_contacts.to_numpy()[0]):
                contact_data = scene.rigid_solver.collider.collider_global_info.contact_data[i, 0]
...
def test_convexify(euler, backend, show_viewer, gjk_collision):
    ...
    assert not gs_sim.rigid_solver.collider.static_collider_info.has_nonconvex_nonterrain
...
def test_data_accessor(n_envs, batched, tol):
    ...
    for _ in range(400):
        ...
        gs_n_contacts = gs_sim.rigid_solver.collider.collider_global_info.n_contacts.to_numpy()
