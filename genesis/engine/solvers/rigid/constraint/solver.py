...
def add_collision_constraints(
    ...
    if qd.static(static_rigid_sim_config.constraint_layout_transposed):
        ...
        # Legacy threading: 1 thread per (i_col, i_b); each thread processes all 4 friction bases inline
        # so per-(i_col, i_b) setup (link walks, d1/d2, invweight) is shared across the 4 frictions.
        # Adjacent lanes vary i_b under flat_idx % _B, coalesced under canonical (n_constraints, n_dofs, _B).
        qd.loop_config(
            name="add_collision_constraints", serialize=static_rigid_sim_config.para_level < gs.PARA_LEVEL.ALL
        )
