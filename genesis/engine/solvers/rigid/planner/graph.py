...
def func_planner_rrt_edge_is_free(
    ...
    for i_dp in range(n_dp):
        reach += planner_info.dof_reach[i_dp] * qd.abs(
            planner_state.rrt_qpos[i_dp, col_to] - planner_state.rrt_qpos[i_dp, col_from]
        )
    ...
    n_sub = gs.qd_int(qd.ceil(4.0 * reach / qd.max(planner_info.eps_act[None], 1e-3))) + 1
    swp = 0.5 * reach / qd.cast(n_sub, gs.qd_float)
    dq_inf = gs.qd_float(0.0)
    for i_dp in range(n_dp):
        dq_inf = qd.max(
            dq_inf,
            qd.abs(planner_state.rrt_qpos[i_dp, col_to] - planner_state.rrt_qpos[i_dp, col_from])
    ...
    while is_free and i_sub <= n_sub:
        ...
        for i_dp in range(n_dp):
            planner_state.eval_qpos[i_dp, i_t] = planner_state.rrt_qpos[i_dp, col_from] + alpha * (
                planner_state.rrt_qpos[i_dp, col_to] - planner_state.rrt_qpos[i_dp, col_from]
            )
...
def kernel_planner_rrt_connect(
    ...
    for i_t in range(envs_idx.shape[0] * n_trees):
        if trees_is_active[i_t] == 1:
            ...
            for i_dp in range(n_dp):
                planner_state.rrt_qpos[i_dp, col0] = planner_info.qpos_start[i_dp, i_b]
                planner_state.rrt_qpos[i_dp, col0 + n_half] = planner_info.qpos_goal[i_dp, i_b]
            planner_state.rrt_parent[col0] = -1
            planner_state.rrt_parent[col0 + n_half] = -1
            planner_state.rrt_n_nodes[2 * i_t] = 1
            planner_state.rrt_n_nodes[2 * i_t + 1] = 1
            planner_state.rrt_is_done[i_t] = False
            planner_state.rrt_path_len[i_t] = 0

            side = 0
            it = 0
            while it < n_iters and not planner_state.rrt_is_done[i_t]:
                base = side * n_half
                other = (1 - side) * n_half
                ...
                if gu.qd_hash01(planner_info.seed_key[None], i_t, it, 777) < _RRT_GOAL_BIAS:
                    newest = other + planner_state.rrt_n_nodes[2 * i_t + 1 - side] - 1
                    for i_dp in range(n_dp):
                        planner_state.eval_qpos[i_dp, i_t] = planner_state.rrt_qpos[i_dp, col0 + newest]
                else:
                    for i_dp in range(n_dp):
                        u = gu.qd_hash01(planner_info.seed_key[None], i_t, it, i_dp)
                        q = planner_info.q_limit_lower[i_dp] + u * (
                            planner_info.q_limit_upper[i_dp] - planner_info.q_limit_lower[i_dp]
                        )
                        if planner_info.dof_is_locked[i_dp, i_b]:
                            q = planner_info.qpos_start[i_dp, i_b]
                        planner_state.eval_qpos[i_dp, i_t] = q

                # Nearest node of the growing side (deterministic lowest-index tie-break).
                i_near = 0
                d_near = gs.qd_float(qd.math.inf)
                for i_n in range(planner_state.rrt_n_nodes[2 * i_t + side]):
                    d = gs.qd_float(0.0)
                    for i_dp in range(n_dp):
                        d += (planner_state.eval_qpos[i_dp, i_t] - planner_state.rrt_qpos[i_dp, col0 + base + i_n]) ** 2
                    if d < d_near:
                        d_near = d
                        i_near = i_n
                ...
                for i_dp in range(n_dp):
                    d_inf = qd.max(
                        d_inf,
                        qd.abs(planner_state.eval_qpos[i_dp, i_t] - planner_state.rrt_qpos[i_dp, col0 + base + i_near]),
                    )
                scale = qd.min(1.0, _RRT_STEER_STEP / qd.max(d_inf, 1e-9))
                i_new = planner_state.rrt_n_nodes[2 * i_t + side]
                if i_new < n_half:
                    col_new = col0 + base + i_new
                    for i_dp in range(n_dp):
                        q_near = planner_state.rrt_qpos[i_dp, col0 + base + i_near]
                        planner_state.rrt_qpos[i_dp, col_new] = q_near + scale * (
                            planner_state.eval_qpos[i_dp, i_t] - q_near
                        )
                    if func_planner_rrt_edge_is_free(
                        ...
                        planner_state.rrt_parent[col_new] = base + i_near
                        planner_state.rrt_n_nodes[2 * i_t + side] = i_new + 1

                        # Connect attempt: nearest node of the other tree tries to join the new node.
                        i_join = 0
                        d_join = gs.qd_float(qd.math.inf)
                        for i_n in range(planner_state.rrt_n_nodes[2 * i_t + 1 - side]):
                            d = gs.qd_float(0.0)
                            for i_dp in range(n_dp):
                                d += (
                                    planner_state.rrt_qpos[i_dp, col_new]
                                    - planner_state.rrt_qpos[i_dp, col0 + other + i_n]
                                ) ** 2
                            if d < d_join:
                                d_join = d
                                i_join = i_n
                        if func_planner_rrt_edge_is_free(
                            ...
                            if side == 0:
                                planner_state.rrt_bridge[i_t] = qd.Vector([base + i_new, other + i_join], dt=gs.qd_int)
                            else:
                                planner_state.rrt_bridge[i_t] = qd.Vector([other + i_join, base + i_new], dt=gs.qd_int)
                            planner_state.rrt_is_done[i_t] = True
                side = 1 - side
                it += 1

            # Path extraction: start-tree chain reversed in place, then the goal-tree chain appended.
            if planner_state.rrt_is_done[i_t]:
                n_path = 0
                node = planner_state.rrt_bridge[i_t][0]
                while node != -1:
                    for i_dp in range(n_dp):
                        planner_state.rrt_path[i_dp, col0 + n_path] = planner_state.rrt_qpos[i_dp, col0 + node]
                    n_path += 1
                    node = planner_state.rrt_parent[col0 + node]
                for i_swap in range(n_path // 2):
                    for i_dp in range(n_dp):
                        tmp = planner_state.rrt_path[i_dp, col0 + i_swap]
                        planner_state.rrt_path[i_dp, col0 + i_swap] = planner_state.rrt_path[
                            i_dp, col0 + n_path - 1 - i_swap
                        ]
                        planner_state.rrt_path[i_dp, col0 + n_path - 1 - i_swap] = tmp
                node = planner_state.rrt_bridge[i_t][1]
                ...
                for i_cut in range(_RRT_N_SHORTCUT):
                    if n_path > 3:
                        u0 = gu.qd_hash01(planner_info.seed_key[None], i_t, i_cut, 555)
                        u1 = gu.qd_hash01(planner_info.seed_key[None], i_t, i_cut, 556)
                        i_from = gs.qd_int(u0 * qd.cast(n_path - 3, gs.qd_float))
                        i_to = i_from + 2 + gs.qd_int(u1 * qd.cast(n_path - i_from - 3, gs.qd_float))
                        for i_dp in range(n_dp):
                            planner_state.rrt_qpos[i_dp, col0] = planner_state.rrt_path[i_dp, col0 + i_from]
                            planner_state.rrt_qpos[i_dp, col0 + 1] = planner_state.rrt_path[i_dp, col0 + i_to]
                        if func_planner_rrt_edge_is_free(
                            i_t,
                            i_b,
                            col0,
                            ...
                            for i_n in range(i_from + 1, n_path - n_cut):
                                for i_dp in range(n_dp):
                                    planner_state.rrt_path[i_dp, col0 + i_n] = planner_state.rrt_path[
                                        i_dp, col0 + i_n + n_cut
                                    ]
                            n_path -= n_cut
                planner_state.rrt_path_len[i_t] = n_path
