...
def ti_transform_by_quat(v, quat):
    # FIXME: Assuming normalized quaternion

    # qvec = ti.Vector([quat[1], quat[2], quat[3]])
    # uv = qvec.cross(v)
    # uuv = qvec.cross(uv)
    # return v + 2.0 * (quat[0] * uv + uuv)
...
def _np_quat_to_R(quat: np.ndarray, out: np.ndarray | None = None) -> np.ndarray:
    ...
    :param quat: N-dimensional array whose last dimension gathers the 4 quaternion coordinates (qx, qy, qz, qw).
...
def _tc_R_to_quat(R, out=None):
    ...
    out[mask1, 0] = 0.25 * S
    out[mask1, 1] = (R[mask1, 2, 1] - R[mask1, 1, 2]) / S
    out[mask1, 2] = (R[mask1, 0, 2] - R[mask1, 2, 0]) / S
    out[mask1, 3] = (R[mask1, 1, 0] - R[mask1, 0, 1]) / S

    S = 2.0 * torch.sqrt(1.0 + diag[mask2, 0] - diag[mask2, 1] - diag[mask2, 2])
    out[mask2, 0] = (R[mask2, 2, 1] - R[mask2, 1, 2]) / S
    out[mask2, 1] = 0.25 * S
    out[mask2, 2] = (R[mask2, 0, 1] + R[mask2, 1, 0]) / S
    out[mask2, 3] = (R[mask2, 0, 2] + R[mask2, 2, 0]) / S

    S = 2.0 * torch.sqrt(1.0 + diag[mask3, 1] - diag[mask3, 0] - diag[mask3, 2])
    out[mask3, 0] = (R[mask3, 0, 2] - R[mask3, 2, 0]) / S
    out[mask3, 1] = (R[mask3, 0, 1] + R[mask3, 1, 0]) / S
    out[mask3, 2] = 0.25 * S
    out[mask3, 3] = (R[mask3, 1, 2] + R[mask3, 2, 1]) / S

    S = 2.0 * torch.sqrt(1.0 + diag[mask4, 2] - diag[mask4, 0] - diag[mask4, 1])
    out[mask4, 0] = (R[mask4, 1, 0] - R[mask4, 0, 1]) / S
    out[mask4, 1] = (R[mask4, 0, 2] + R[mask4, 2, 0]) / S
    out[mask4, 2] = (R[mask4, 1, 2] + R[mask4, 2, 1]) / S
    out[mask4, 3] = 0.25 * S
...
def trans_R_to_T(trans=None, R=None):
    assert trans is not None or R is not None
    ...
    B = R.shape[:-2] if trans is None else trans.shape[:-1]
...
def R_to_T(R):
    return trans_R_to_T(None, R)


def trans_to_T(trans):
    return trans_R_to_T(trans, None)
...
@nb.jit(nopython=True, cache=True)
def z_up_to_R(v_a: np.ndarray, up: np.ndarray | None = None, out: np.ndarray | None = None) -> np.ndarray:
    """
    ...
    for i in np.ndindex(B):
        ...
        if up is None:
            ...
            cos_theta[i] = np.dot(v_a, up[i])
            if abs(cos_theta[i]) < 1.0 - gs.EPS:
                axis_i[:] = np.cross(up[i], z)
            else:
                axis_i[:] = 0.0, 1.0, 0.0
                axis_i -= np.dot(v_a, axis_i) * v_a
