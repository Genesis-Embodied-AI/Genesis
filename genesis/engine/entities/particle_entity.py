...
class ParticleEntity(Entity):
    ...
    def _sanitize_particles_idx_local(self, particles_idx_local, *, unsafe=False):
        ...
        if unsafe:
            return particles_idx_local, envs_idx
        ...
        return _particles_idx_local, envs_idx
    ...
    def _add_vverts_to_solver(self):
        ...
        # FIXME: This stack patterns looks fishy
        breakpoint()
        Ps = np.stack(
    ...
    @assert_active
    def _set_particles_target_state(self, key, name, element_shape, tensor, envs_idx=None, *, unsafe=False):
        if self.sim.requires_grad:
            gs.logger.warning(
                f"Manually setting particle '{key}'. This is not recommended and could break gradient flow."
            )

        envs_idx = self._scene._sanitize_envs_idx(envs_idx, unsafe=unsafe)
        element_ndim, element_expend = len(element_shape), ((1,) * element_shape)

        tensor = to_gs_tensor(tensor)
        if tensor.ndim == element_ndim:
            tensor = tensor.reshape((1, 1, *element_shape)).expand((len(envs_idx), self._n_particles, *element_expend))
        elif tensor.ndim == element_ndim + 1 and len(tensor) == len(envs_idx):
            tensor = tensor.reshape((1, self._n_particles, *element_shape)).expand((len(envs_idx), 1, *element_expend))
        elif tensor.ndim == element_ndim + 1 and len(tensor) == self._n_particles:
            tensor = tensor.reshape((len(envs_idx), 1, 3)).expand((1, self._n_particles, *element_expend))
        if tensor.ndim != element_ndim + 2:
            gs.raise_exception(f"Invalid tensor shape ({tensor.shape}).")

        self._tgt[key] = tensor
    ...
    def set_com_pos(self, pos, envs_idx=None, *, unsafe=False):
        ...
        self._set_particles_target_state("pos", "position", (3,), poss, envs_idx, unsafe=True)
    ...
    def set_particles_pos(self, poss, envs_idx=None, *, unsafe=False):
        ...
        envs_idx : None | int | array_like, shape (M,), optional
            The indices of the environments to set. If None, all environments will be considered. Defaults to None.
        """
        self._set_particles_target_state("pos", "position", (3,), poss, envs_idx, unsafe=unsafe)
    ...
    def set_particles_vel(self, vels, envs_idx=None, *, unsafe=False):
        ...
        envs_idx : None | array_like, optional
            The indices of the environments to set. If None, all environments will be considered. Defaults to None.
        """
        self._set_particles_target_state("vel", "velocity", (3,), vels, envs_idx, unsafe=unsafe)
    ...
    def set_particles_active(self, actives, envs_idx=None, *, unsafe=False):
        ...
        envs_idx : None | int | array_like, shape (M,), optional
            The indices of the environments to set. If None, all environments will be considered. Defaults to None.
        """
        self._set_particles_target_state("act", "activeness", (), actives, envs_idx, unsafe=unsafe)
    ...
    def find_closest_particle(self, pos, envs_idx=None, *, unsafe=False):
        ...
        distances = torch.linalg.norm(cur_particles - torch.as_tensor(pos, dtype=gc.tc_float, device=gs.device), dim=1)
