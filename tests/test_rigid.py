...
ENABLE_INTERACTIVE_VIEWER = True
...
class TestRigid(unittest.TestCase):
    ...
    def _check_mujoco_consistency(self):
        gs_meaninertia = self.mj_model.stat.meaninertia
        mj_meaninertia = self.gs_solver.meaninertia.to_numpy()[0]
        ...
        if gs_n_constraints:
            ...
            mj_contact_pos = -self.mj_data.contact.frame[:, :3]
            np.testing.assert_allclose(gs_contact_normal[gs_sidx], mj_contact_pos[mj_sidx], atol=1e-9)
            ...
            gs_sidx = np.argsort(gs_jac.cumsum(1).std(1))
            mj_sidx = np.argsort(mj_jac.cumsum(1).std(1))
    ...
    def _simulate_and_check_mujoco_consistency(self, qpos, qvel):
        self._init_engines(qpos, qvel)

        for i in range(150):
            self._check_mujoco_consistency()
