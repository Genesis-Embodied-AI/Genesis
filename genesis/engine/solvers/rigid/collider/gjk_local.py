...
def func_gjk_local(
    ...
    for i in range(gjk_info.gjk_max_iterations[None]):
        ...
        if n == 3 and backup_gjk:
            ...
            if intersect_code == gjk.GJK_RETURN_CODE.SEPARATED:
                ...
                early_stop = True
                break
            elif intersect_code == gjk.GJK_RETURN_CODE.INTERSECT:
                # Intersection found
                nx = 0
                dist = 0.0
                nsimplex = 4
        ...
        _lambda = gjk.func_gjk_subdistance(gjk_state, gjk_info, i_b, n + 1)
        ...
        next_support_vector = gjk.func_simplex_vertex_linear_comb(gjk_state, i_b, 2, 0, 1, 2, 3, _lambda, n)
        if gjk.func_is_equal_vec(next_support_vector, support_vector, gjk_info.FLOAT_MIN[None]):
    ...
    if not early_stop:
        ...
        for i in range(2):
            witness_point = gjk.func_simplex_vertex_linear_comb(gjk_state, i_b, i, 0, 1, 2, 3, _lambda, nsimplex)
            if i == 0:
...
def func_gjk_intersect_local(
    ...
    flag = gjk.GJK_RETURN_CODE.NUM_ERROR
    for i in range(gjk_info.gjk_max_iterations[None]):
        ...
        for j in range(4):
            ...
            n, s = gjk.func_gjk_triangle_info(gjk_state, gjk_info, i_b, s0, s1, s2)
        ...
        if min_sdist >= 0:
            ...
            flag = gjk.GJK_RETURN_CODE.INTERSECT
        ...
        if is_no_collision:
            flag = gjk.GJK_RETURN_CODE.SEPARATED
            break
