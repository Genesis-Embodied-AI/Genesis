...
class RigidEntity(Entity):
    ...
    def _load_scene(self, morph, surface):
        ...
        if (
            ...
            and self.material.ipc_coup_type == "ipc_only"
            and any(l_info["is_robot"] for l_info in l_infos)
        ):
            gs.raise_exception(
                "`RigidMaterial.ipc_coup_type='ipc_only'` only supported by rigid non-articulated objects."
            )
    ...
    def set_pos(self, pos, envs_idx=None, *, zero_velocity=True, relative=False):
        ...
        if (
            isinstance(self.sim.coupler, IPCCoupler)
            and self.material.ipc_coup_type is not None
            and self.base_link.is_fixed
        ):
            gs.raise_exception(
                "This method is only supported by `RigidMaterial.ipc_coup_type=None` for fixed-based rigid entities."
            )
    ...
    def set_qpos(self, qpos, qs_idx_local=None, envs_idx=None, *, zero_velocity=True, skip_forward=False):
        ...
        if isinstance(self.sim.coupler, IPCCoupler) and self.material.ipc_coup_type == "external_articulation":
            gs.raise_exception("This method is not supported by `RigidMaterial.ipc_coup_type='external_articulation'`.")
    ...
    def control_dofs_force(self, force, dofs_idx_local=None, envs_idx=None):
        ...
        if isinstance(self.sim.coupler, IPCCoupler) and self.material.ipc_coup_type == "ipc_only":
            gs.raise_exception("This method is not supported for `ipc_coup_type='ipc_only'` entities.")
